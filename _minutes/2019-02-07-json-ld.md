...
> ***Resolution #8: Update the algorithm to describe desired behavior of disallowing unbounded loading of contexts, still allow ping-ponging between two contexts in the document (or scoped contexts).*** {: #resolution8 .resolution}
...
> ***Resolution #9: Add JSON data type to RDF, with support in JSON-LD processors for managing parsed JSON in the internal form. We will seek feedback via blogpost, and in Berlin.*** {: #resolution9 .resolution}
...
* [Resolution #3](#resolution3): Close #98, as @context: null clears context definitions
* [Resolution #4](#resolution4): Close #98, as @context: null clears context definitions
* [Resolution #5](#resolution5): if `@id` is not present in a term definition, and there is no default vocabulary, but has an expansion to an absolute IRI in the active context, then the `@id` of the new definition is taken from the active context (otherwise this is still an error)
* [Resolution #6](#resolution6): Once previous is done, we can close #116
* [Resolution #7](#resolution7): We agree with the processing order per #61:  scoped definition from property, then from type, then embedded. PR to update docs to come.
* [Resolution #8](#resolution8): Update the algorithm to describe desired behavior of disallowing unbounded loading of contexts, still allow ping-ponging between two contexts in the document (or scoped contexts).
* [Resolution #9](#resolution9): Add JSON data type to RDF, with support in JSON-LD processors for managing parsed JSON in the internal form. We will seek feedback via blogpost, and in Berlin.
